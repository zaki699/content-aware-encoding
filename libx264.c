...
typedef struct X264Context {
    ...
    char *x264opts;
    float crf;
    float current_crf; // Current CRF value, updated per frame
    float crf_max;
    int cqp;
    int aq_mode;
    float aq_strength;
...
static int process_frame_metadata_and_reconfig_encoder(AVCodecContext *ctx, const AVFrame *frame);
                      ...
                      int *got_packet)
{
    ...
    if (frame) {
        // Check for custom CRF side data
        AVFrameSideData *crf_side_data = av_frame_get_side_data(frame, AV_FRAME_DATA_CUSTOM_CRF);
        if (crf_side_data) {
            int new_crf;
            memcpy(&new_crf, crf_side_data->data, sizeof(int));

            // Update the encoder parameters
            x264_param_t new_params = x4->params;
            new_params.rc.f_rf_constant = new_crf;

            int ret = x264_encoder_reconfig(x4->enc, &new_params);
            if (ret < 0) {
                av_log(ctx, AV_LOG_ERROR, "Failed to reconfigure x264 encoder with new CRF.\n");
                // Handle error appropriately
            } else {
                av_log(ctx, AV_LOG_INFO, "Successfully reconfigured x264 encoder with CRF = %d.\n", new_crf);
                // Update stored parameters
                x4->params = new_params;
            }
        }
    }
...
static av_cold int X264_init(AVCodecContext *avctx)
{
    ...
    if (avctx->flags & AV_CODEC_FLAG_PASS2) {
        ...
        if (x4->crf >= 0) {
            x4->params.rc.i_rc_method   = X264_RC_CRF;
            x4->params.rc.f_rf_constant = x4->crf;
            // Initialize current_crf with the default CRF value
            x4->current_crf = x4->crf;
        } else if (x4->cqp >= 0) {
